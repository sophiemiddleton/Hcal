...
class HcalGeometryProvider : public ConditionsObjectProvider {
 public:
  /**
   * Class constructor
   * @param parameters -- uses the "HcalGeometry" section to configure the HcalGeometry
   */	
 ...
 private:
  ...
  HcalGeometry* hcalGeometry_;
};
    

HcalGeometryProvider::HcalGeometryProvider(const std::string& name, const std::string& tagname, const Parameters& parameters, Process& process) : ConditionsObjectProvider{HcalGeometry::CONDITIONS_OBJECT_NAME,tagname,parameters,process}, params_{parameters} {
...
std::pair<const ConditionsObject*,ConditionsIOV> HcalGeometryProvider::getCondition(const EventHeader& context) {
  ...
  if (!hcalGeometry_) {
    ...
    Parameters phex=(params_.exists("HcalGeometry"))?(params_.getParameter<Parameters>("HcalGeometry")):(params_);
    ...
    for (auto key: phex.keys()) {
     ...
     std::vector<std::string> dets_valid=pver.getParameter<std::vector<std::string> >(KEYNAME);
      for (auto detregex : dets_valid) {
        ...
        regfree(&reg);
        if (!nmatch) {
          hcalGeometry_=new HcalGeometry(pver);
          break;
        }
      }
